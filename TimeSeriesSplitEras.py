...
class TimeSeriesSplitEras(_BaseKFold):
    ...
    def split(self, X, y=None, groups=None):
        ...
        # Build era-to-index mapping via groupby — O(n_rows) instead of O(n_eras * n_rows)
        era_to_idx = {era: idx.values for era, idx in X.groupby(self.era_col).groups.items()}
        ...
        # Calculate test size; last fold absorbs remainder so no eras are wasted
        ...
        for i in range(self.n_splits):
            # Calculate test period — last fold gets all remaining eras
            test_start = initial_train_size + (i * test_size_eras)
            if i == self.n_splits - 1:
                test_end = n_eras  # Last fold absorbs remainder
            else:
            ...
            train_eras = unique_eras[0:train_end]
